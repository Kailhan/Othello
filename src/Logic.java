public class Logic {
    ...
    public static boolean checkSquareAllowed(int x, int y, Board board, int playerNr) //Returns true if the position clicked would result in disks being flipped
    {
        ...
        if (boardGrid[x][y] == 0) //If the square is empty
        {
            if (x > 1  && x < boardSize - 2 && y > 1 && y < boardSize - 2) { //middle
                if(nrFlippedDirection(x, y, board, playerNr, "N").length > 0 ||
                        nrFlippedDirection(x, y, board, playerNr, "NE").length > 0 ||
                        nrFlippedDirection(x, y, board, playerNr, "E").length > 0 ||
                        nrFlippedDirection(x, y, board, playerNr, "SE").length > 0 ||
                        nrFlippedDirection(x, y, board, playerNr, "S").length > 0 ||
                        nrFlippedDirection(x, y, board, playerNr, "SW").length > 0 ||
                        nrFlippedDirection(x, y, board, playerNr, "W").length > 0 ||
                        nrFlippedDirection(x, y, board, playerNr, "NW").length > 0)
                    return true;
            }
                ...
                if(nrFlippedDirection(x, y, board, playerNr, "E").length > 0 ||
                        nrFlippedDirection(x, y, board, playerNr, "SE").length > 0 ||
                        nrFlippedDirection(x, y, board, playerNr, "S").length > 0 ||
                        nrFlippedDirection(x, y, board, playerNr, "SW").length > 0 ||
                        nrFlippedDirection(x, y, board, playerNr, "W").length > 0)
                ...
                if(nrFlippedDirection(x, y, board, playerNr, "S").length > 0 ||
                        nrFlippedDirection(x, y, board, playerNr, "SW").length > 0 ||
                        nrFlippedDirection(x, y, board, playerNr, "W").length > 0)
                    return true;
            }
    ...
    public static int[][] nrFlippedDirection(int x, int y, Board board, int playerNr, String direction)
    {
        int[][] boardGrid = board.getBoardGrid();
        int boardSize = board.getSize();

        boolean foundEnemy = false;
        int[][] enemyCoordinates = new int[0][2];
                ...
                for(int i = 1; y - i >= 0; i++)
                    {
                        ...
                        else if(boardGrid[x][y - i] == playerNr * -1)
                        {
                            ...
                            enemyCoordinates = combine(enemyCoordinates, enemyCoordinate);
                        }
